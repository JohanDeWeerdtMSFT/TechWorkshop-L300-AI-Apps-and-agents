...
{: .important }
> With the current package versions in this lab environment, enabling `OpenAIInstrumentor` or module-level Azure Monitor setup in imported tool modules can break Foundry tool-call paths with a `NonRecordingSpan` error. Keep Azure Monitor configuration centralized in `chat_app.py` unless you have validated newer SDK versions end to end. The deployed lab app keeps Application Insights enabled and leaves the tool-call paths stable.
