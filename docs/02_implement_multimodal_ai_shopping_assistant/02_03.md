...
{: .note }
> The Container App also needs Microsoft Foundry project access when it calls Foundry agents with Microsoft Entra ID. Assign the **Foundry User** role to the Container App managed identity on both the Foundry resource and Foundry project scopes. The Foundry RBAC roles were recently renamed, so when using Azure CLI or infrastructure code, prefer the role definition ID for Foundry User: `53ca6127-db72-4b80-b1b0-d745d6d5456d`.
