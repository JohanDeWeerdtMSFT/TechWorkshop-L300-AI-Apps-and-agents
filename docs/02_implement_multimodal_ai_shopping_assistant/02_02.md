...
Alternatively, if your repository includes the consolidated deployment helper, you can create all six agents from the `src` directory with one command:

```bash
python app/agents/deploy_agents.py
```

{: .note }
> You may receive an error reading, in part, **Message: The principal `{YOUR_PRINCIPAL_ID}` lacks the required data action `Microsoft.CognitiveServices/accounts/AIServices/agents/write`**. If you receive this error message, return to your resource group and choose the Microsoft Foundry resource associated with this training (that is, not the project). Navigate to **Access control (IAM)** from the left-hand menu. Select the **+ Add** button and then choose the **Add role assignment** option. In the **Role** dropdown, select the **Foundry User** role. The Foundry roles were recently renamed, so when scripting this assignment use the role definition ID `53ca6127-db72-4b80-b1b0-d745d6d5456d` instead of relying on the display name. In the **Assign access to** dropdown, select **+ Select members**. In the **Select members** list, choose your name. After that, select the **Select** button at the bottom of the pane. Finally, select **Review + assign** twice to grant the Foundry User role to your account. Then, re-run the command.
